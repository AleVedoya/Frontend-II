...
function crearCard(noticia, contenedor) {
  if (noticia.tipoNacional) {
    origen = "Noticia Nacional";
  } else {
    origen = "Noticia Internacional";
  }

  const card = ` <article class="article" >
                    <img src="${noticia.imgUrl}" alt="IMAGEN" />
                    <div class="noticia">
                      <h2 class="news-title">Título: ${noticia.titulo}</h2>
                      <p class="description">${noticia.descripcion}</p>
                      <div class="details">
                          <p class="date">${noticia.fecha}</p>
                          <p class="nac">${origen}</p>
                      </div>
                    </div>
                  </article>`;
